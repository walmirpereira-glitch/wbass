...
export function HeroSection() {
  ...
  return <section id="hero" className="relative w-full">
      ...
      <div className="relative w-full h-[60vh] md:h-[80vh] lg:h-[90vh] overflow-hidden">
        <AnimatePresence mode="wait">
          ...
          duration: 0.8
            ...
            <img src={slide.image} alt={`Wbass Cabinets - ${slide.title}`} className="w-full h-full object-cover" />
          </motion.div>
        </AnimatePresence>

        {/* Navigation Arrows - Quilter Labs style */}
        <button onClick={prevSlide} className="absolute left-4 md:left-8 top-1/2 -translate-y-1/2 z-10 p-3 bg-background/90 hover:bg-background rounded-full shadow-lg transition-all hover:scale-110" aria-label="Slide anterior">
          <ChevronLeft className="w-6 h-6 text-foreground" />
        </button>
        <button onClick={nextSlide} className="absolute right-4 md:right-8 top-1/2 -translate-y-1/2 z-10 p-3 bg-background/90 hover:bg-background rounded-full shadow-lg transition-all hover:scale-110" aria-label="Próximo slide">
          <ChevronRight className="w-6 h-6 text-foreground" />
        </button>

        {/* Slide Indicators */}
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-10 flex gap-3">
          {heroSlides.map((_, idx) => <button key={idx} onClick={() => setCurrentSlide(idx)} className={`w-3 h-3 rounded-full transition-all ${idx === currentSlide ? "bg-background scale-125" : "bg-background/50 hover:bg-background/80"}`} aria-label={`Ir para slide ${idx + 1}`} />)}
        </div>
      </div>
      ...
      <div className="bg-background py-12 md:py-16">
        <div className="container mx-auto px-6 lg:px-12 text-center">
          ...
          delay: 0.2
            ...
            <a href="#products">
              <Button size="xl" className="bg-primary hover:bg-primary/90 text-primary-foreground font-semibold px-8">
                Ver Produtos
              </Button>
            </a>
            <a href="#about">
              <Button variant="outline" size="xl" className="border-border text-foreground hover:bg-muted font-semibold px-8">
                Saiba Mais
              </Button>
            </a>
          </motion.div>
            ...
            duration: 0.6
              ...
              <div className="relative aspect-square overflow-hidden rounded-lg shadow-xl">
                ...
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
                  {shuffledGallery.map((_, idx) => <button key={idx} onClick={() => setCurrentGalleryIndex(idx)} className={`w-2 h-2 rounded-full transition-all ${idx === currentGalleryIndex ? "bg-primary scale-125" : "bg-background/70 hover:bg-background"}`} aria-label={`Ver foto ${idx + 1}`} />)}
                </div>
              </div>
            </motion.div>
          </div>
